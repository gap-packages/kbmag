...
static fsa *fsa_wa_short(fsa *fsaptr, storage_type op_table_type,
                         boolean destroy, char *tempfilename);
static fsa *fsa_wa_int(fsa *fsaptr, storage_type op_table_type, boolean destroy,
                       char *tempfilename);

fsa *fsa_wa(fsa *fsaptr, storage_type op_table_type, boolean destroy,
            char *tempfilename)
{
                         ...
                         boolean destroy, char *tempfilename)
{
  ...
  if (fsaptr->table->table_type != DENSE) {
    fprintf(stderr, "Error: function fsa_wa can only be called with a "
                    "densely-stored fsa.\n");
    return 0;
  }
