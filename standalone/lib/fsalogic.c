...
{ int i, j, l, *il1, *il2;
  ...
  if (srptr1->type==IDENTIFIERS || srptr1->type==STRINGS) {
    ...
    for (i=1;i<=srptr1->size;i++) {
      il1=srptr1->intlist[i]; il2=srptr2->intlist[i];
      l=il1[0];
      for (j=0;j<=l;j++) if (il1[j]!=il2[j])
        return FALSE;
    }
  }
	...
	char *tempfilename;
{
  ...
  return 
    fsa_binop(fsaptr1,fsaptr2,op_table_type,destroy,tempfilename,AND,FALSE);
}
	...
	char *tempfilename;
{
  ...
  return 
    fsa_binop(fsaptr1,fsaptr2,op_table_type,destroy,tempfilename,AND,TRUE);
}
	...
	char *tempfilename;
{
  ...
  return 
    fsa_binop(fsaptr1,fsaptr2,op_table_type,destroy,tempfilename,OR,FALSE);
}
 ...
 */
{
  int **itable, **table, ne, ns, nt1, dr, nt, *sparseptr, iin, ina,
      im, i, ct,  g;
  boolean dense_ip;
  fsa *star;
 ...
 */
{
  int **table1, **table2, **table, ne, ns1, ns2, nt1, nt2, dr1, dr2,
      nt, *sparseptr, in2, na2, im, i, g;
...
/* We won't rely on fsaptr1/2->table->numTransitions being stored */
  nt1=0;
  for (g=1;g<=ne;g++) for (i=1;i<=ns1;i++)
        if (target(dense_ip1,table1,g,i,dr1) != 0)
          nt1++;
  nt2=0;
  for (g=1;g<=ne;g++) for (i=1;i<=ns2;i++)
 ...
 */
  ...
  if (fsa_table_dptr_init(greater_than)==-1) return 0;
