...
static int make_substitution(gen *w, int start, int end,
  ...
  while (reducible) {
    ...
    for (i = start; i <= end; i++) {
      if ((state = dense_target(watable, w[i], state)) == 0) {
        ...
        if (make_substitution(w, start, end, rs_wd) == -1)
          return -1;
        ;
        break;
      }
                             ...
                             reduction_struct *rs_wd)
{
  int ndiff, ngens, identity, padsymbol, wordlen, ***difftab, gct, *gpref,
      level, gen1, gen2, diff, diffct, newdiff, shift, cwordwt, csubwt, i, j;
  ...
  */
  ...
  gptr[0].genno = 0;
  gptr[0].diffno = identity;
  gptr[0].subwt = 0;
  gptr[0].lexprec = FALSE;
  gptr[0].backptr = 0;
  ...
  while (1) {
    ...
    */
    for (diffct = level == start ? 0 : gpref[level - 2] + 1;
         diffct <= gpref[level - 1]; diffct++) {
      cstruc = gptr + diffct;
      diff = cstruc->diffno;
      for (gen2 = 1; gen2 <= ngens + 1; gen2++) {
        if ((level == start && gen2 == gen1) ||
            (gen1 == padsymbol && gen2 == padsymbol) ||
         ...
         */
        if (level >= end &&
            (csubwt > cwordwt || (csubwt == cwordwt && !clexprec)))
          continue;
              ...
              */
              for (i = gpref[level - 1] + 1;; i++) {
                ...
                if (substruc->diffno == newdiff) {
                  if (csubwt < substruc->subwt ||
                      (csubwt == substruc->subwt && cstruc->lexprec &&
                       !substruc->lexprec)) {
