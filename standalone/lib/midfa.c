 ...
 */
fsa *midfa_determinize(fsa *fsaptr, storage_type op_table_type, boolean destroy,
                       char *tempfilename)
{
  if (kbm_print_level >= 3)
    printf("    #Calling midfa_determinize.\n");
  if (fsaptr->states->size < MAXUSHORT)
    return midfa_determinize_short(fsaptr, op_table_type, destroy,
                                   tempfilename);
  else
...
int midfa_minimize(fsa *fsaptr)
{
  ...
  if (fsaptr->table->table_type == SPARSE && fsaptr->table->denserows > 0) {
    fprintf(stderr, "Sorry: midfa_minimize unavailable for sparse storage with "
                    "dense rows.\n");
    return -1;
  }
  if (kbm_print_level >= 3)
...
int midfa_labeled_minimize(fsa *fsaptr)
{
  ...
  if (fsaptr->table->table_type == SPARSE && fsaptr->table->denserows > 0) {
    fprintf(stderr, "Sorry: midfa_labeled_minimize unavailable for sparse "
