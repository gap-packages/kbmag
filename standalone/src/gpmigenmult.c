...
{ int arg, i, ct, *inv, ngens, maxwdiffs, maxeqns;
  ...
  migenmultptr = fsa_mitriples(&wa,&diff2,op_store,TRUE,tempfilename,eqnptr,
...
/* We need to know the inverses of generators - let's just work them out! */
      ...
      while (eqnptr[i].lhs && i<maxeqns) {
        if (add_wd_fsa_cos(&diff1,eqnptr+i,inv,FALSE,&rs_wd)== -1) exit(1);
        i++;
      }
      tfree(diff1.initial);
  ...
  if (readback) {
    if (midfa_labeled_minimize(migenmultptr)== -1) exit(1);
  }
/*
  else
