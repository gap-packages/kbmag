...
static int maxsubgens = MAXSUBGENS, maxwordlen = MAXWORDLEN,
           maxwords = MAXWORDS;

static char gpname[100], subname[100], inf[100], outf[100], fsaname[100],
    tempfilename[100];
static fsa subwa, genmult, mult, mult1, mult2, *compmultptr, *submultptr,
    *existsptr;
...
static int subgen_multiplier(gen *w, char *s);
static void read_subgpgens(void);
static int output_bad_words(void);
...
int main(int argc, char *argv[])
{
  ...
  fclose(rfile);
  /* Since this now has states of type labeled, we need to remove this
   * structure, or fsa_equal will not work in the correctness test.
   */
  ...
  for (i = 0; i < nsubgens; i++) {
    ...
    if (!gotsuff) {
      if (subgen_multiplier(subgen[i], suff) == -1)
        exit(1);
    }
    ...
    submultptr =
        fsa_submult(&subwa, &mult, op_store, FALSE, tempfilename, readback);
    if (submultptr == 0)
      exit(1);
    fsa_clear(&mult);
    if (kbm_print_level > 1)
    ...
    if (readback) {
      if (fsa_minimize(submultptr) == -1)
        exit(1);
    }
    else {
      if (fsa_ip_minimize(submultptr) == -1)
        exit(1);
    }
    if (kbm_print_level > 1)
    ...
    /* As usual, we output and input again, in case we want to change storage
     * types.
     */
    base_prefix(fsaname);
...
void read_subgpgens(void)
{
  ...
  nsubgens = 0;
  do {
    if (!read_word(rfile, testword, testword + maxwordlen, &delim, names, ngens,
                   TRUE)) {
      fprintf(stderr,
...
int output_bad_words(void)
{
  ...
  tmalloc(badwords, gen *, maxwords) numwords =
      words_and_not(&subwa, existsptr, badwords, maxwords);
  if (numwords == -1)
    return -1;
  if (numwords == 0) {
...
void badusage(void)
{
  ...
  fprintf(stderr, "\t\t[-m maxwords] groupname [subname].\n");
  exit(1);
}
