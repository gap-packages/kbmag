...
int main(int argc, char *argv[])
{
  ...
  while (calc_wa) {
    ...
    while (calc_gm) {
      ...
      genmultptr =
          cosets
              ? fsa_mitriples(wa, &diff2, op_store_gm, TRUE, tempfilename,
                              eqnptr, maxeqns1, eqnstop, &foundeqns, readback)
              : fsa_triples(wa, &diff2, op_store_gm, TRUE, tempfilename, eqnptr,
                            maxeqns1, eqnstop, &foundeqns, readback);

      if (foundeqns) {
        /*This is the case where the calculation failed and new equations was
         * found. If the -diff1 flag was set, we correct the groupname.(mi)diff1
         * machine.
         */
        if (correction) {
          if (kbm_print_level > 1)
            printf("  #Altering first wd-machine to make it accept new "
                   "equations.\n");
          ...
          while (eqnptr[i].lhs && i < maxeqns1) {
            ...
            else if (add_wd_fsa(&diff1, eqnptr + i, inv, FALSE, &rs_wd) == -1)
              exit(1);
            i++;
          }
          if (cosets) {
      ...
      if (kbm_print_level > 1) {
        if (cosets)
          printf("  #Number of states of mi-multiplier before minimisation = "
                 "%d.\n",
      ...
      if (readback) {
        if (cosets) {
          if (midfa_labeled_minimize(genmultptr) == -1)
            exit(1);
        }
        else if (fsa_labeled_minimize(genmultptr) == -1)
          exit(1);
      }
      ...
      if (cosets) {
        ...
        tfree(genmultptr);
        genmultptr =
            migm_determinize(&migenmult, op_store_gm, TRUE, tempfilename);
        if (kbm_print_level > 1)
          printf(
              "  #Number of states of multiplier before minimisation = %d.\n",
              genmultptr->states->size);
                                       ...
                                       separator)) > 0)
      {
        ...
        fsa_clear(genmultptr);
        if (kbm_print_level > 1)
          printf("  #Altering second wd-machine to make it accept new "
                 "equations.\n");
         ...
         * The right hand side of the equation to be added will be the reduction
         * of the lhs times the generator which is currently in the lhs.
         */
        for (i = 0; i < numeqns; i++) {
          genstrcat(eqnptr[i].lhs, eqnptr[i].rhs);
          tfree(eqnptr[i].rhs);
          /* In the cosets case, we need to allow some extra room for the
           * reduction of the word, since it might acquire a long prefix in the
           * subgroup.
           */
          if (cosets)
            tmalloc(
...
void badusage(void)
{
  fprintf(stderr, "Usage: gpmakefsa [-opwa d/s] [-opgm d/s] [-ip d/s[dr]]\n");
  fprintf(stderr, "\t\t [-silent] [-v] [-l/-h] [-diff1/-diff2] [-m maxeqns]\n");
  fprintf(stderr,
